...
public class CoreRebecaStatementTransformer extends AbstractStatementTransformer {
	...
	protected String resolveStatement(Statement statement) {
		...
		//Should be added by Ehsan
		if (statement instanceof MoveStatement)
		{
			return resolveMoveStatement((Move) statement);
		}
	...
	protected String resolveMoveStatement(MoveStatement moveStatement) {
		String retValue = "";
		// The Move parameters should be managed in this section
		// Then the move command should be published on the topic related to robot's cmd channel
		return retValue;
	}
